...
```sh
https://github.com/satyamkodale/URL-Shortener
```
...
The server will start listening on port `1911` by default. You can change the port in the `main.go` file if needed.

### Shortening a URL

To shorten a URL, send a POST request to the `/short_url` endpoint with a JSON payload containing the original URL:

```sh
curl -X POST http://localhost:1911/shorten -H "Content-Type: application/json" -d '{"url": "https://example.com"}'
```
...
```json
{
    "short_url": "https://www.linkedin.com/in/satyamkodale/"
}
```
